...
export async function enrichLaunch(ev: LaunchEvent, ctx: DeployerIndex | EnrichContext = {}): Promise<LaunchProfile> {
  ...
  const flowBlocks = [...buys.map((b) => b.blockNumber), ...sells.map((s) => s.block)];
  const flowSpan = flowBlocks.length > 1
    ? Number(flowBlocks.reduce((max, block) => block > max ? block : max, flowBlocks[0]) - flowBlocks.reduce((min, block) => block < min ? block : min, flowBlocks[0]))
    : 0;
  const flowWindowSeconds = flowSpan > 0 ? flowSpan * config.blockSeconds : null;
  const flow = computeFlow(buys, sells, qMeta.decimals, flowWindowSeconds);
