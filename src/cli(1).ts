...
import { render, toJson, renderRewind, renderBoard, renderRotation } from "./render.js";
...
import { buildIntelligence } from "./momentum.js";
...
function recordSnapshot(profile: LaunchProfile, score: number, critical: boolean): void {
  ...
  history.snapshot(profile.event.token, {
    ...
    liquidityQuote: profile.market?.liquidityQuote ?? null,
    critical,
  });
  history.save();
}
...
async function show(profile: LaunchProfile, json: boolean, minScore: number): Promise<OpportunityRead | null> {
  ...
  decorate(profile, result);
  console.log(json ? toJson(profile, result) : render(profile, result));
  return profile.opportunity;
}

function decorate(profile: LaunchProfile, result: ReturnType<typeof scoreProfile>): void {
  ...
  const intelligence = buildIntelligence(profile, history);
  profile.momentum = intelligence.momentum;
  profile.graduation = intelligence.graduation;
  profile.poolHealth = intelligence.poolHealth;
  profile.capitalRotation = intelligence.capitalRotation;
}
...
program.command("demo").description("show a sample radar card without network access").action(() => {
  ...
  const flow = computeFlow(samples, demoSells, QDEC, 18);
...
program.command("rotation").description("rank recent launches by observed capital flow").option("--blocks <n>", "lookback blocks", "2000").option("--top <n>", "number of rows", "10").action(async (opts: { blocks: string; top: string }) => {
  await index.build(Number(opts.blocks));
  ...
  console.log(radarBanner("backfill"));
  console.log(renderRotation(profiles, Number(opts.top)));
});
